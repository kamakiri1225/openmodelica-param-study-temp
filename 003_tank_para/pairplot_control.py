...
def main():
    ...
    Tfin_on = Tair + dT_on
    labels = ["最終水温 [degC]", "発熱量Q [W]", "heatCeffToAir", "外気温 [degC]",
              "体積 [L]", "表面積 [m²]", "水位 [m]", "温度上昇 ΔT [K]", "5τ [h]"]

    data_off = np.column_stack([Tfin_off, Q, h_air, Tair, V, Ar, level, dT_off, tau5])
    data_on = np.column_stack([Tfin_on, Q, h_air, Tair, V, Ar, level, dT_on, tau5])
