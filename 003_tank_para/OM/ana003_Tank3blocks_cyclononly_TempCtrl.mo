model ana003_Tank3blocks_cyclononly_TempCtrl
  ...
  // === 温度管理レギュレータ (ctrl_k=0で無効=なし, >0で有効=あり。目標=外気温) ===
  parameter Real Ttarget_deg = Tair_deg "温度管理の目標温度[degC]";
  parameter Real ctrl_k = 0 "温度管理ゲイン (0=なし, >0=あり)";
  ...
  Modelica.Blocks.Sources.Constant Treg_target(k = Ttarget_deg + 273.15) annotation(
    Placement(transformation(origin = {-270, 130}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Math.Feedback Treg_fb annotation(
    Placement(transformation(origin = {-230, 110}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Continuous.PID Treg_pid(k = ctrl_k, Ti = 200, Td = 0, initType = Modelica.Blocks.Types.Init.InitialOutput, y_start = 0) annotation(
...
equation
  connect(Treg_sensor.port, tank1.heatPort);
  connect(Treg_target.y, Treg_fb.u1);
  connect(Treg_sensor.T, Treg_fb.u2);
  connect(Treg_fb.y, Treg_pid.u);
  connect(Treg_pid.y, Treg_Q.Q_flow);
  connect(Treg_Q.port, tank1.heatPort);
  ...
  annotation(
    ...
    version = "");
end ana003_Tank3blocks_cyclononly_TempCtrl;
