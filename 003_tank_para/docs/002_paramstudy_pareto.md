...
## 5b. pairplot（設計変数＋各熱流＋体積）

設計変数（発熱量 Q・heatCeffToAir・水位）に加え、定常状態の**各熱流**と**体積**も
...
heatCeffToAir↔Tmax −0.69、側/底放熱↔Tmax 0.68。

読み取り例：
- **水位 ↔ 体積**：ほぼ比例（体積 = 断面積×水位）。
- **Q ↔ 上面→大気 / 側底→地面**：ともに正（入熱が増えると放熱も増えて釣り合う）。
- **側/底→地面放熱 ↔ Tmax**：正の相関（地面放熱が大きい条件ほど…等、条件依存を可視化）。
- **水位・体積 ↔ 5τ**：正（水量が多いほど整定が遅い）。
- **heatCeffToAir ↔ Tmax**：負（放熱を上げると飽和温度が下がる）。
