// ============================================================
//  eva4_09_loop2to3_split   (eva5_04 全部品化 + 温度管理 TempControl)
...
// ============================================================
model eva4_09_loop2to3_split
  extends Modelica.Icons.Example;
  import Modelica.Fluid.Vessels.BaseClasses.VesselPortsData;
  //--- 再利用部品: タンク内の混ざりにくい部分(よどみ部)。本体とは少ない混合流量でだけ熱をやり取りする ---
...
時定数 m_zone*4186/(mix_Lmin/60*4186) で遅れて追従する。</p></html>"));
  ...
  //--- 再利用部品: 温度管理(センサ=吸込口)(冷却)ループ tank1→tank3 ---
  model TempControlIn "tank1から引いてtank3へ返す温度管理(冷却)ループ。PIDで除熱し目標温度に保持"
    ...
    Modelica.Fluid.Sensors.Temperature Tsens(redeclare package Medium = Medium) "吸込(タンクから引いた水)の温度" annotation(
  ...
  equation
    ...
    connect(port_a, Tsens.port) annotation(Line(points = {{-100, 0}, {-60, 0}, {-60, 20}, {30, 20}, {30, 30}}, color = {0, 127, 255}));
...
Tsensで水温を測り、PID(ctrl_k)がcoolerで除熱してTtargetに保持する。ctrl_k=0で管理OFF。</p></html>"));
  ...
  //25;
  parameter Real T_ini = 23.8 + 273.15 "初期水温(eva4実測の開始時)";
  //25 + 273.15;
  // 機械内
  ...
  Modelica.Fluid.Vessels.OpenTank tank1(height = tank_height, crossArea = Lx1_1*Ly1_1*(1 - f_zone), level_start = level_start, T_start = T_ini, portsData = {VesselPortsData(diameter = diamDmyPipe, height = 0), VesselPortsData(diameter = diamDmyPipe, height = 0), VesselPortsData(diameter = diamDmyPipe, height = 0.12)}, redeclare package Medium = fluid1, use_T_start = true, use_HeatTransfer = true, nPorts = 3) annotation(
  ...
  GroundLoss groundLoss2(Gc_liquid = heatCefftTank2in*(Lx2_1*Ly2_1 + Lx2_2*Ly2_2 + Ly2_1*level_start + Ly2_2*level_start + Lx2_1*level_start), G_wall = (Lx2_1*Ly2_1 + Lx2_1*level_start + Ly2_1*level_start)*kground/tank_thickness, Gc_ground = heatCeffToAir*(Lx2_1*Ly2_1 + Lx2_2*Ly2_2 + Ly2_1*level_start + Ly2_2*level_start + Lx2_1*level_start)) annotation(
    Placement(transformation(origin = {58, -78}, extent = {{-20, -20}, {20, 20}})));
  ...
  AirLoss airLoss1(Gc_air = heatCeffToAir*(Lx1_1*Ly1_1)) annotation(
    Placement(transformation(origin = {30, 92}, extent = {{-20, -20}, {20, 20}})));
  AirLoss airLoss2(Gc_air = heatCeffToAir*(Lx2_1*Ly2_1 + Lx2_2*Ly2_2)) annotation(
    Placement(transformation(origin = {88, 46}, extent = {{-20, -20}, {20, 20}})));
  AirLoss airLoss3(Gc_air = heatCeffToAir*(Lx3_1*Ly3_1)) annotation(
    Placement(transformation(origin = {208, 40}, extent = {{-20, -20}, {20, 20}})));
  TempControlIn tempControl(redeclare package Medium = fluid1, Ttarget = Ttarget, ctrl_k = 2000, cooling_capacity = 3500, flow_Lmin = 25, T_start = T_ini) annotation(
    Placement(transformation(origin = {148, 86}, extent = {{-20, -20}, {20, 20}})));
  parameter Real f_zone = 0.7 "tank1 のうち、混ざりにくい部分(よどみ部)の割合";
  StagnantZone zone1(m_zone = f_zone*Lx1_1*Ly1_1*level_start*995, mix_Lmin = 0.15, T_init = T_ini - 273.15) annotation(
    Placement(transformation(origin = {-38, 34}, extent = {{-16, -16}, {16, 16}})));
...
equation
  connect(airLoss1.port_water, tank1.heatPort) annotation(
    Line(points = {{10, 92}, {10, 45}, {-8, 45}, {-8, 14}}, color = {191, 0, 0}));
  ...
  connect(groundLoss2.port_water, tank2.heatPort) annotation(
    Line(points = {{38, -78}, {38, 20}, {120, 20}}, color = {191, 0, 0}));
  ...
  connect(airLoss2.port_water, tank2.heatPort) annotation(
    Line(points = {{88, 36}, {88, 20}, {120, 20}}, color = {191, 0, 0}));
  connect(airLoss3.port_water, tank3.heatPort) annotation(
  ...
  connect(groundLoss2.port_ground, prescribedTemperature.port) annotation(
    Line(points = {{78, -78}, {78, -186}, {-10, -186}}, color = {191, 0, 0}));
  connect(groundLoss1.port_ground, prescribedTemperature.port) annotation(
    Line(points = {{-22, -108}, {-10, -108}, {-10, -186}}, color = {191, 0, 0}));
  connect(airLoss2.port_air, prescribedTemperature.port) annotation(
    Line(points = {{88, 56}, {80, 56}, {80, 148}, {-200, 148}, {-200, -156}, {-10, -156}, {-10, -186}}, color = {191, 0, 0}));
  connect(airLoss1.port_air, prescribedTemperature.port) annotation(
    Line(points = {{50, 92}, {50, 150}, {-212, 150}, {-212, -158}, {-10, -158}, {-10, -186}}, color = {191, 0, 0}));
  connect(airLoss3.port_air, prescribedTemperature.port) annotation(
    Line(points = {{208, 50}, {204, 50}, {204, 150}, {-210, 150}, {-210, -156}, {-10, -156}, {-10, -186}}, color = {191, 0, 0}));
  connect(tank2.ports[3], tempControl.port_a) annotation(
    Line(points = {{140, 0}, {140, -12}, {90, -12}, {90, 86}, {128, 86}}, color = {0, 127, 255}, thickness = 0.5));
  connect(tempControl.port_b, tank3.ports[1]) annotation(
    Line(points = {{168, 86}, {248, 86}, {248, -6}, {246, -6}}, color = {0, 127, 255}, thickness = 0.5));
  connect(zone1.port, tank1.heatPort) annotation(
    Line(points = {{-22, 34}, {-22, 31}, {-8, 31}, {-8, 14}}, color = {191, 0, 0}, thickness = 0.5));
  annotation(
    uses(Modelica(version = "4.0.0")),
    Diagram(coordinateSystem(extent = {{-260, 400}, {420, -240}})),
    version = "1.0.0");
end eva4_09_loop2to3_split;
