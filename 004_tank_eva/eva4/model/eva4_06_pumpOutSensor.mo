// ============================================================
//  eva4_06_pumpOutSensor   (eva5_04 全部品化 + 温度管理 TempControl)
...
// ============================================================
model eva4_06_pumpOutSensor
  extends Modelica.Icons.Example;
  import Modelica.Fluid.Vessels.BaseClasses.VesselPortsData;
  //--- 再利用部品: センサ位置を選べるオイルコン(冷却機側の小さな水の塊を冷やし、その水かポンプ出口の温度で制御) ---
...
冷却のみ、除熱上限 capacity。enabled=falseで除熱0。</p></html>"));
  ...
  SensorCooler sCooler(Ttarget = 24.5, capacity = 3500, m_local = 10, flow_Lmin = 25, sensorAtPumpOut = true, enabled = true) annotation(
    Placement(transformation(origin = {12, 92}, extent = {{-16, -16}, {16, 16}})));
  Modelica.Fluid.Sensors.Temperature T_pumpOut(redeclare package Medium = fluid1) "ポンプ出口温度" annotation(
...
equation
  ...
  connect(sCooler.port, tank1.heatPort) annotation(
    Line(points = {{12, 76}, {12, 14}}, color = {191, 0, 0}, thickness = 0.5));
  connect(T_pumpOut.port, pumpHeat.port_b) annotation(
