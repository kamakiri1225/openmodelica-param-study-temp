# -*- coding: utf-8 -*-
"""eva5: OpenModelica 結果と実測の比較。
先に run_sim.mos を実行して ana003_Tank3blocks_cyclononly_NoTemp_res.csv を生成しておく。
OM結果が無ければ実測のみ描画する。"""
import os, csv
import numpy as np
...
AMB='4-9'
EXP=os.path.join(HERE,"data","temperature_20points.csv")
OMRES=os.path.join(HERE,"ana003_Tank3blocks_cyclononly_NoTemp_res.csv")
...
if os.path.exists(OMRES):
    ...
    print(f"eva5 RMSE={rmse:.3f} degC")
else:
    print("OM結果CSVが見つかりません。先に run_sim.mos を実行してください:",OMRES)

ax.set_title("eva5 OM計算 vs 実測"+rmse_txt)
